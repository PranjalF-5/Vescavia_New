  ...
  return (
    <section id={SectionId.ABOUT} className="py-24 md:py-32 bg-vescavia-light dark:bg-vescavia-black relative z-10 transition-colors duration-300 overflow-hidden">
      <div className="container mx-auto px-6">
        ...
        <div className="mb-32">
          <div className="flex md:grid md:grid-cols-3 gap-6 md:gap-8 overflow-x-auto snap-x snap-mandatory pb-6 md:pb-0 -mx-6 px-6 md:mx-0 md:px-0">
            {values.map((item, i) => (
              <motion.div
                key={item.title}
                ...
                className="min-w-[80vw] md:min-w-0 snap-center p-8 border border-black/10 dark:border-white/10 bg-white dark:bg-white/5 rounded-2xl hover:border-vescavia-purple/50 dark:hover:border-vescavia-purple/50 transition-colors group"
              >
                <div className="w-12 h-12 rounded-full bg-black/5 dark:bg-white/10 flex items-center justify-center mb-6 text-black dark:text-white group-hover:bg-vescavia-purple group-hover:text-white transition-colors duration-300">
                  {item.icon}
                </div>
          ...
          <div className="flex md:grid md:grid-cols-3 gap-6 md:gap-8 overflow-x-auto snap-x snap-mandatory pb-6 md:pb-0 -mx-6 px-6 md:mx-0 md:px-0">
            {team.map((member, i) => (
              ...
              >
                ...
                <span className="text-xs font-mono text-gray-500 uppercase tracking-widest group-hover:text-vescavia-purple transition-colors">{member.role}</span>
              </motion.div>
            ))}
          </div>
        </motion.div>
